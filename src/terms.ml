...
let get_xs (defs : (atom * ftype * ('a, 'b, 'c, 'd, 'e, 'f) _fterm) list) :
  atom list =
...
let get_typed_xs (defs : (atom * ftype * ('a, 'b, 'c, 'd, 'e, 'f) _fterm) list)
  : (atom * ftype) list =
  List.map (fun (x, expected, _) -> (x, expected)) defs


(* [get_terms1 defs] returns the list of let rec term definitions *)

let get_terms1s (defs : (atom * ftype * ('a, 'b, 'c, 'd, 'e, 'f) _fterm) list) :
  ('a, 'b, 'c, 'd, 'e, 'f) _fterm list =
  List.map (fun (_, _, terms1) -> terms1) defs
...
let get_typss
  (defs :
    ...
    * ('a, 'b, 'c, 'd, 'e, 'f) _fterm )
    list ) : atom list list =
  List.map (fun (_, typs, _, _, _) -> typs) defs
...
let get_varss
  (defs :
    ...
    * ftype
    * ('a, 'b, 'c, 'd, 'e, 'f) _fterm )
    list ) : (atom * ftype) list list =
  List.map (fun (_, _, vars, _, _) -> vars) defs
...
let get_expecteds
  (defs :
    ...
    * ('a, 'b, 'c, 'd, 'e, 'f) _fterm )
    list ) : ftype list =
  List.map (fun (_, _, _, expected, _) -> expected) defs
